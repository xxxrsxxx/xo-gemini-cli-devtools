...
export interface DevToolsWidgetProps {
  ...
  apiKey?: string;
  /**
   * Force enable or disable the widget.
   * If not provided, it defaults to showing only in development environments
   * (process.env.NODE_ENV !== 'production').
   */
  enabled?: boolean;
}

const WidgetContent: React.FC<DevToolsWidgetProps> = ({ enabled, apiKey }) => {
  ...
  // If enabled is explicitly passed, respect it.
  // Otherwise, check if we are NOT in production.
  const isDevelopment = typeof process !== 'undefined' && process.env && process.env.NODE_ENV !== 'production';
  const shouldRender = enabled !== undefined ? enabled : isDevelopment;

  useEffect(() => {
    if (shouldRender) {
      if (apiKey) {
        console.log('DevToolsWidget: API Key received.');
      }
...
export const DevToolsWidget: React.FC<DevToolsWidgetProps> = (props) => {
  return (
      <LanguageProvider>
        <WidgetContent {...props} />
      </LanguageProvider>
